...
function mapDict<T, S>(
  inputVal: Dict<T>,
  callback: (original: T, key: string) => S
): Dict<S> {
  const outDict: Dict<S> = {};

  for (let key of Object.keys(inputVal)) {
    const value = inputVal[key];
    outDict[key] = callback(value, key);
  }
...
function filterDict<T>(
  inputVal: Dict<T>,
  callback: (original: T) => boolean
): Dict<T> {
  const outDict: Dict<T> = {};

  for (const k of Object.keys(inputVal)) {
    const value = inputVal[k];
    if (callback(value)) {
      outDict[k] = value;
    }
  }
...
function reduceDict<T, S>(
  inputVal: Dict<T>,
  callback: (acc: S, value: T) => S,
  initialValue: S
): S {
  let value = initialValue;
  for (const k of Object.keys(inputVal)) {
    const thisVal = inputVal[k];
    value = callback(value, thisVal);
  }
